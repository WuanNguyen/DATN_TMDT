...
class _AdminStatisticsScreenState extends State<AdminStatisticsScreen> {
  ...
  Widget build(BuildContext context) {
    for(int month = 1;month <=12; month++){
      ...
      MonthlyRevenue[month-1].setSale(double.parse(stats['monthlyRevenue']?[month.toString().padLeft(2, '0')]?.toStringAsFixed(2) ?? '0.00'));
      MonthlySold[month-1].setSale(double.parse(stats['monthlySold']?[month.toString().padLeft(2, '0')]?.toStringAsFixed(2) ?? '0.00'));
    };
    print(double.parse(stats['monthlySold']?[2.toString().padLeft(2, '0')]?.toStringAsFixed(2) ?? '0.00'));
    return Scaffold(
      appBar: AppBar(
...
//------------------------------------------------------
List<SalesData> MonthlyRevenue = List.generate(12,(index) => SalesData('T${index+1}', 0));
List<SalesData> MonthlySold = List.generate(12,(index) => SalesData('T${index+1}', 0));
//------------------------------------------------------
